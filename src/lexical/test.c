...
int main(int argc, char **argv) {
  while (true) {
    ...
    if (val == IDENT) printf("%s", yytext);

    if (val == NUMBER) print_num();

    if (val == STRING)
      print_escaped_string(yylval.data.string.string,
                           yylval.data.string.length);

    if (val == CHARLIT) print_escaped_char(yylval.data.character);
