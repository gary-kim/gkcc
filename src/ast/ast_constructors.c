...
struct ast_node *ast_node_new_for_loop(struct ast_node *expr1,
  ...
  node->for_loop.expr1 =
      expr1 == NULL ? ast_node_new_list_node(ast_node_new_constant_int_node(1))
                    : expr1;
  node->for_loop.expr2 =
      expr2 == NULL ? ast_node_new_list_node(ast_node_new_constant_int_node(1))
                    : expr2;
  node->for_loop.expr3 =
      expr3 == NULL ? ast_node_new_list_node(ast_node_new_constant_int_node(1))
                    : expr3;
...
struct ast_node *ast_node_update_struct_or_union_specifier_node(
  ...
  if (members != NULL) {
    gkcc_assert(node->gkcc_type.gkcc_type->ident == NULL ||
                    node->gkcc_type.gkcc_type->ident->ident.symbol_table_entry
                            ->fully_defined == false,
                GKCC_ERROR_REDEFINITION,
